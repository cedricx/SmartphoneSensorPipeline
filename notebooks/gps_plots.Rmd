...
```{r age and sex}
age_sex_gps_fit = gam(gps ~ s(age) + sex + gps_days, data = acc_subj_mood_df, method = "REML")
age_sex_acc_fit = gam(acc ~ s(age) + sex + acc_days, data = acc_subj_mood_df, method = "REML")
age_sex_accgps_fit = gam(accgps ~ s(age) + sex + acc_days + gps_days, data = acc_subj_mood_df, method = "REML")

age_sex_gps_res = gam(gps ~  s(age) + gps_days, data = acc_subj_mood_df, method = "REML")
age_sex_acc_res = gam(acc ~  s(age)  + acc_days, data = acc_subj_mood_df, method = "REML")
age_sex_accgps_res = gam(accgps ~ s(age)  + acc_days + gps_days, data = acc_subj_mood_df, method = "REML")


#age
anno_text = paste("r=",round(cor.test(age_sex_accgps_res$residuals,age_sex_accgps_fit$model$age)$estimate,2), ", p=", round(summary(age_sex_accgps_fit)[[8]],3))

#sex
anno_text = paste("r=",round(sqrt(summary(lm(update(age_sex_accgps_fit,~.-sex)$residuals ~ age_sex_accgps_fit$model$sex))$r.squared),2), ", p=", round(summary(age_sex_accgps_fit)[[4]][2],4))

#gps_days
anno_text = paste("r=",round(sqrt(summary(lm(update(age_sex_acc_fit,~.-acc_days)$residuals ~ age_sex_acc_fit$model$acc_days))$r.squared),2), ", p=", round(summary(age_sex_acc_fit)[[4]][3],4))

p = visreg(age_sex_accgps_fit, "sex", gg = T, line=list(col="#3576b5",size = 0.5, alpha = 0.25), 
...
pdf(file.path(paste0(output_path,"sex_gpsacc_scatter_noage.pdf")),height = 4,width = 5)
print(p)
dev.off()
...
```{r som_som}
p = visreg(sig_fc_fit, "accgps", gg = T, line=list(col="#3576b5",size = 0.5), 
       points=list( size=2, pch=19), fill= list(fill=c("#b3d3f2"))) + 
  xlab("Individual Footprint Distinctiveness") + ylab("Within Somatomotor Network FC") + 
  theme_cowplot() + 
  annotate("text", -Inf, Inf, label = "R = 0.27, p = 0.046", hjust = -0.1, vjust = 1)

pdf(file.path(paste0(output_path,"fc_som_accgps.pdf")),height = 4,width = 5)
print(p)
dev.off()
```