...
fc_com_gps = function(subj_net_files_1,subj_net_files_2,atlas_now){
  ...
  fc_com_corplot = sim_plots(subj_seq, ids,names(fc_com_1) , fc_com_1, fc_com_2)
  fc_com_cor_r = lapply(fc_com_corplot, function(atlas) atlas$sim_mat$r[lower.tri(atlas$sim_mat$r)])
  fc_com_gps_pvalue = unlist(sapply(fc_com_cor_r, function(atlas) cor.test(atlas, gps_corplot$r[lower.tri(gps_corplot$r)]))[c('p.value'),])
  fc_com_gps_cor = unlist(sapply(fc_com_cor_r, function(atlas) cor.test(atlas, gps_corplot$r[lower.tri(gps_corplot$r)]))[c('estimate'),])
  fc_com_gps_cor_df = data_frame(atlas = gsub("\\..*","",names(fc_com_gps_cor)), corr = fc_com_gps_cor, p_val = fc_com_gps_pvalue )
  ...
  p = ggplot(data=fc_com_gps_cor_df, aes(x=reorder(atlas, -corr), y=corr, fill = sig)) +
    geom_bar(stat="identity") + scale_y_continuous(limits=c(-0.5,0.5)) +
...
fc_com_gps_out_corplot = function(fc_com_gps_out, title) {
  ...
  com_gps_pval_mat = vector_to_mat(fc_com_gps_out$fc_gps_df$p_val_adj,"half+d")
