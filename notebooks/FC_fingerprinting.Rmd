...
for (subj in arrange(subj_days,`Days Collected`)$IID){
    ...
    for (part in 1:1){
      ...
      gps_wide_matrix = rbind(gps_wide_matrix,half_1_half_2)
      }
    #}
}
...
fc_com_gps = function(subj_net_files_1,subj_net_files_2,atlas_now){
  ...
  fc_com_gps_cor_df$p_perm_adj = p.adjust(fc_com_gps_cor_df$p_perm,method = "fdr")
...
get_acc_finger = function(subj_net_files_1,subj_net_files_2){
  ...
  for (atlas in atlases) {
    ...
    for(subj in names(cor_match[[atlas]])) {
       ...
       target_list[[atlas]][[subj]] = subj
       match_list [[atlas]][[subj]] = predicted_subj
       correct_list[[atlas]][[subj]] = F
       if (predicted_subj == subj) {
            acc[atlas]= acc[atlas] + 1
            correct_list[[atlas]][[subj]] = T
          }
    }
    
    cor_df[[atlas]] = data.frame(target = unlist(target_list[[atlas]]), match = unlist(match_list[[atlas]]),  finger = unlist(cor_list[[atlas]]), correct = unlist(correct_list[[atlas]]))
...
```{r bootstrapping for fingerprint matching}
get_acc_fc = function(target,database){
  match_acc = 0
  predicted_subj = list()
  cor_match = lapply(target, function(subj_cor_1) sapply(database, function(subj_cor_2) cor(subj_cor_1$V1, subj_cor_2$V1, use = "na.or.complete")))
  for (target_subj in names(target)){
    predicted_subj[[target_subj]] = names(which.max(cor_match[[target_subj]]))
    if (target_subj == predicted_subj[[target_subj]]){
      match_acc = match_acc +1
    }
  }
  match_df = data.frame(target = names(target), predict = unname(unlist(predicted_subj)))
  match_df$correct = F
  match_df$correct[which(names(target) == unname(unlist(predicted_subj)))] = T
  match_acc = match_acc / length(target)
  return(list(acc = match_acc, df = match_df))
}


bt_seqs = createDataPartition(1:41, times = 100, p= 0.666, list = T)
acc_bt = list()

for (atlas in atlases){
  for (i in 1:length(bt_seqs)){
      print(paste(atlas,i))
      specific_bt = bt_seqs[[i]]
      subj_net_bt_1 = subj_net_files_1[[atlas]][specific_bt]
      subj_net_bt_2 = subj_net_files_2[[atlas]][specific_bt]
      acc_bt[[atlas]][[i]] = get_acc_fc(subj_net_bt_1,subj_net_bt_2)
    }
  }
}
...
```{r combine bootstrap results}
fc_acc_all = lapply(acc_bt,function(atlas) sapply(atlas, function(bt) bt$acc))
fc_acc_predict = lapply(acc_bt,function(atlas) t(sapply(atlas, function(bt) bt$df[,'predict'])))
fc_acc_target = lapply(acc_bt,function(atlas) t(sapply(atlas, function(bt) bt$df[,'target'])))
fc_acc_subj = lapply(acc_bt,function(atlas) t(sapply(atlas, function(bt) bt$df)))
fc_acc_df = lapply(fc_acc_subj, function(atlas) data.frame(target = unlist(atlas[,"target"]),
                                               correct = unlist(atlas[,"correct"])))
fc_acc_subj_mean = lapply(fc_acc_df, function(atlas) atlas %>% group_by(target) %>% summarise(mean = mean(correct)))
sapply(fc_acc_subj, function(atlas) )
```

```{r correlate fc subj acc with gps subj acc, fig.height=2, fig.width=2}
atlas_now = "aal116"
id_dict = data.frame(bblid = as.character(ids$BBLID), beiweid = as.character(ids$beiweID))
fc_acc_subj_atlas = data.frame(bblid = fc_acc_subj_mean[[atlas_now]]$target, fc_acc = fc_acc_subj_mean[[atlas_now]]$mean)
fc_acc_subj_atlas = inner_join(id_dict,fc_acc_subj_atlas)

gps_fc_acc_atlas = inner_join(fc_acc_subj_atlas, psych_sum, by = c("beiweid" = "beiweID"))

fit = lm(fc_acc ~ acc + admin_age + admin_sex + days, data = gps_fc_acc_atlas)
visreg(fit,"acc")
```
```{r}
atlas_now = "schaefer200x7"
fc_com_1  = get_fc_com(atlas_now, subj_net_files_1)
fc_com_2  = get_fc_com(atlas_now, subj_net_files_2)
acc_com = get_acc_fc(fc_com_1,fc_com_2)
for (i in 1:length(bt_seqs)){
    specific_bt = bt_seqs[[i]]
    subj_net_bt_1 = subj_net_files_1[[atlas_now]][specific_bt]
    subj_net_bt_1_atlas = list()
    subj_net_bt_1_atlas[[atlas_now]] = subj_net_bt_1
    subj_net_bt_2 = subj_net_files_2[[atlas_now]][specific_bt]
    subj_net_bt_2_atlas = list()
    subj_net_bt_2_atlas[[atlas_now]] = subj_net_bt_2
    
    fc_com_1  = get_fc_com(atlas_now, subj_net_bt_1_atlas)
    fc_com_1  = get_fc_com(atlas_now, subj_net_bt_2_atlas)
...
com_com_400 = list()
for (com_com in names(which(fc_schaef400_7_com_gps$fc_gps_df$sig_perm == T))) {
  com_com_df = data.frame(fc = fc_schaef400_7_com_gps$fc_corplot[[com_com]]$sim_mat$r[lower.tri(fc_schaef400_7_com_gps$fc_corplot[[com_com]]$sim_mat$r)],gps = gps_corplot$r[lower.tri(gps_corplot$r)])
  com_com_400[[com_com]] = ggplot(com_com_df, aes(x = fc, y = gps)) + geom_bin2d(bins=50) + theme_cowplot() + xlab(paste(com_com,"(FC)")) + ylab(paste(com_com,"(GPS)"))
}
Reduce(`+`, com_com_400)
```
...
com_com_200 = list()
for (com_com in names(which(fc_schaef200_7_com_gps$fc_gps_df$sig_perm == T))) {
  com_com_df = data.frame(fc = fc_schaef200_7_com_gps$fc_corplot[[com_com]]$sim_mat$r[lower.tri(fc_schaef200_7_com_gps$fc_corplot[[com_com]]$sim_mat$r)],gps = gps_corplot$r[lower.tri(gps_corplot$r)])
  com_com_200[[com_com]] = ggplot(com_com_df, aes(x = fc, y = gps)) + geom_bin2d(bins=50) + theme_cowplot() + xlab(paste(com_com,"(FC)")) + ylab(paste(com_com,"(GPS)"))
}
Reduce(`+`, com_com_200)
```
