...
for (subj in arrange(subj_days,`Days Collected`)$IID){
  ...
  if (subj != ids[ids$BBLID == "21059",'beiweID']) {
    for (part in 1:1){
...
calc_acc_time=function(match_cor,part_times, method = "max") {
  acc_time = array()
  for (time in 1:part_times){
    acc_time[time] = 0
...
calc_acc_subj = function(match_cor, part_times, method = "max"){
  acc_subj = array()
  for (subj in names(subj_mat_1)){
    acc_subj[subj] = 0
    for (time in 1:part_times){
...
```{r run permutation for mean features match by cor, fig.width= 6, message=FALSE}
...
```{r run permutation for mean features match by rmse, fig.width= 6, message=FALSE}
...
```{r run permutation for rmssd features matched by cor, fig.width= 6, message=FALSE}
...
```{r run permutation for rmssd features matched by rmse, fig.width= 6, message=FALSE}
...
```{r subj accuracy cor with potential confounders, fig.height=6, fig.width=7, wmessage=FALSE, warning=FALSE}
...
```{r match with combined features, fig.align="center"}
...
```{r psychopathology , fig.align = "center", fig.height=3, fig.width=6, message=FALSE, warning=FALSE}
...
sp_als = psych_sp(psych_sum, 'sum_als',"Affective Lability Score (ALS)")
sp_ari = psych_sp(psych_sum, 'sum_ari',"Affective Reactivity Index (ARI)")

sp_als + sp_ari
 # show results


fit = lm( acc ~ sum_als + sum_ari+ days, data = psych_sum)
summary(fit)
```
...
### 17. Associations with Cognition
```{r psychopathology, fig.align="center", fig.height=3, fig.width=6, message=FALSE, warning=FALSE}
cnb_score = read.csv(file.path(project_path,"data/grmpy_bifactor_cnb_cleaned.csv"))
cor_traits = read.csv(file.path(project_path,"data/grmpy_corrtraits_cnb_cleaned.csv"))
cnb_beiwe = inner_join(ids,cnb_score, by = c("BBLID" = "bblid"))
