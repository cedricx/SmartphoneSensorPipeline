...
```{r psychopathology, fig.align="center", fig.height=6, fig.width=3, message=FALSE, warning=FALSE}
...
als_fit = lm( acc ~ scale(sum_als) + scale(days) + scale(admin_age) + scale(admin_sex) , data = psych_sum)
ari_fit = lm( acc ~ scale(sum_ari) + scale(days) + scale(admin_age) + scale(admin_sex) , data = psych_sum)
ari_als_fit = lm( acc ~ scale(sum_ari) + scale(sum_als) + scale(days) + scale(admin_age) + scale(admin_sex) , data = psych_sum)
...
p<-ggplot(acc_time_lesion_df, aes(x=reorder(feature, acc_time, FUN = median), y=acc_time)) + 
  ...
  #scale_y_continuous(limits=c(-0.1,0.5)) +
   #ggtitle("Feature Leison Analysis")  +
