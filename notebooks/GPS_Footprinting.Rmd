...
q_var_list[,c("sleep_cat","mood_cat","mood_since_cat")] = cbind(sleep_qs_cat_var,mood_now_qs_cat_var,mood_since_qs_cat_var)
...
get_q_var_pvals = function(question_of_interest){
  ...
  q_var_gps_fit = lm(ifd_gps ~ mood_var + gps_days + age + sex, data = q_var_lm_df)
  q_var_gps_p = summary(q_var_gps_fit)[[4]][,4]['mood_var']
  
  q_var_accgps_fit = lm(ifd_accgps ~ mood_var + gps_days + acc_days + age + sex, data = q_var_lm_df)
  q_var_accgps_p = summary(q_var_accgps_fit)[[4]][,4]['mood_var']
  ...
  return(data.frame(p_gps = q_var_gps_p, p_accgps = q_var_accgps_p, rsqr_gps = q_var_gps_res_cor, rsqr_accgps = q_var_accgps_res_cor))
}

get_q_var_lm = function(question_of_interest){
  q_var_lm_df = data.frame(acc_days = subj_acc_days$acc_days, gps_days = subj_days$`Days Collected`, mood_var = q_var_list[,question_of_interest], ifd_accgps =  value(acc_subj_cb_accgps), ifd_gps = value(acc_subj), age = psych_sum$admin_age, sex = as.factor(psych_sum$admin_sex))
  q_var_gps_p = lm(ifd_gps ~ mood_var + gps_days + age + sex, data = q_var_lm_df)
  q_var_accgps_p = lm(ifd_accgps ~ mood_var  + gps_days + acc_days + age + sex, data = q_var_lm_df)
  
  return(list(gps = q_var_gps_p, accgps = q_var_accgps_p))
}

q_var_pvals = as.data.frame(t(sapply(names(q_var_list), get_q_var_pvals)))
q_var_pvals = q_var_pvals[which(q_var_pvals$p_gps <0.1 | q_var_pvals$p_accgps <0.1),]
...
beiwe_dx = beiwe_dx[-duplicated(beiwe_dx$BBLID),]
...
acc_subj_dx_sum_fit = lm(accgps ~ AxisII_Sum + gps_days + acc_days + sex + age, acc_subj_dx_sum)
