...
LatLong2XY = function(lat_v,lon_v,R=6.371*10^6){
  ...
  d3=  2*pi*R*cos(ph0*2*pi/360)*(th1-th0)/360
  d2=  2*pi*R*cos(ph1*2*pi/360)*(th1-th0)/360
  x_v=rep(0,length(lon_v))
  y_v=rep(0,length(lat_v))
  for(i in 1:length(lat_v)){
    w2=(lat_v[i]-ph0)/(ph1-ph0)
    w1=(lon_v[i]-th0)/(th1-th0)
    #    x_v[i]=w1*abs(d3-d2)/2+w2*(d3*(1-w1)+d2*w1)  # based on trapezoid
    #    y_v[i]=w1*d1*sin(acos(abs((d3-d2)/(2*d1))))
    x_v[i]=w1*d1   # based on square
    y_v[i]=w2*(d3+d2)/2
  }
