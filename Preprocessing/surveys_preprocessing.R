surveys_preprocessing = function(patient_name, ...){
  survey_data = list()
  weekly_data_filepath = paste(data_filepath, patient_name, "beiwe", sep="/")
  if(file.exists(weekly_data_filepath)){
    weekly_names = list.files(weekly_data_filepath)
    for (weekly_name in weekly_names){
      surveys_data_filepath = paste(weekly_data_filepath,weekly_name,"survey_answers",sep='/')
